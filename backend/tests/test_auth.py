...
class TestAuth:
    ...
    def test_login_zle_haslo(self, http_client: httpx.Client) -> None:
        ...
        resp = http_client.post(
            "/auth/login",
            json={"username": "admin", "password": "ZLEHASLO_SELFTEST_XYZ"},
        )
        assert resp.status_code == 401, (
            f"Oczekiwano 401 dla złego hasła, got {resp.status_code}"
        )
    ...
    def test_endpoint_bez_tokenu(self, http_client: httpx.Client) -> None:
        ...
        # Tworzymy nowy klient bez tokenu
    ...
    def test_refresh_token(self, authed_client: httpx.Client) -> None:
        """Odświeżenie tokenu zwraca nowy access_token."""
        resp = authed_client.post("/auth/refresh")
        # 200 = OK, 401 = brak cookie refresh_token (akceptowalny w środowisku testowym)
        assert resp.status_code in (200, 401), (
            f"Oczekiwano 200 lub 401, got {resp.status_code}: {resp.text[:200]}"
        )
    ...
    def test_me(self, authed_client: httpx.Client) -> None:
        ...
        data = resp.json()
        # Sprawdź że jest username w odpowiedzi
    ...
    def test_logout_i_blacklista(self, http_client: httpx.Client) -> None:
        ...
        with httpx.Client(
            ...
            # Token powinien być teraz na blackliście
