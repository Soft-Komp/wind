...
async def list_faktury(
    ...
    approval_status: Optional[str] = Query(
        ...
        pattern="^(in_progress|pending_dispatch|approved|cancelled|rejected)$",
        description="Filtr techniczny po statusie obiegu Sprint 3",
    ),
    routing_status: Optional[str] = Query(
        default=None,
        alias="routing_status",
        pattern="^(to_dispatch)$",
        description="Filtr biznesowy: to_dispatch = approval IS NULL OR status=pending_dispatch",
    ),
